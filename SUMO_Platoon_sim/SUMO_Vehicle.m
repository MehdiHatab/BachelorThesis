...
IDs = get(handles.NearVehicle,'String');
if isa(UD,'inf_veh')
    UD(end+1)   = inf_veh(IDs{idx},Leader.Pos,Leader.Ang);
    UD(end).sig = 1;
else
    UD        = inf_veh(IDs{idx},Leader.Pos,Leader.Ang);
    UD(1).sig = 1;
end
    
% if strcmp(UD.IDs(idx),IDs(idx))
%     UD.Signals(idx) = 1;
% else 
...
%     temp = find(ismemeber(objID(idx),UD.current.IDs));
%     if ~isempty(temp)
...
% handles    structure with handles and user data (see GUIDATA)
UD  = get(handles.output,'UserData');
idx = get(handles.FollowerVehicle,'Value');
IDs = get(handles.FollowerVehicle,'String');

for idx2 = 1:numel(UD)
    if strcmp(UD(idx2).ID,IDs{idx})
        UD(idx2).sig = 2;
    end 
end
set(handles.output,'UserData',UD);
